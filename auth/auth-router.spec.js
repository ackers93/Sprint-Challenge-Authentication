...
describe("GET /jokes", () => {
  ...
  it("should return status 201 on login", async () => {
    ...
    const response = await request(server)
      ...
      .send({ username: "andrew", password: "ackerman" });
  ...
  it("should return a token", async () => {
    ...
    const loginResponse = await request(server)
      .post("/api/auth/login")
      .send({ username: "andrew", password: "ackerman" });
    expect(loginResponse.body.message).toMatch("Welcome andrew!");
    console.log(loginResponse.body);
  });
});
...
describe("POST /register", () => {
  ...
  it("should register member to db", async () => {
    const newDad = await request(server)
      .post("/api/auth/register")
      .send({ username: "andrew", password: "andrew" });
    expect(newDad.body.username).toMatch(/andrew/);
  });
